                ...
                <div class="flex">
                    <div class="max-w-xs">
                        ...
                        <div class="mt-1 relative rounded-md shadow-md">
                            <input v-on:keydown.enter="add" v-model="ticker" type="text" name="wallet" id="wallet"
            ...
            <template v-if="tickers.length > 0">
                ...
                <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
                    <div v-for="t in tickers" :key="t.name" @click="select(t)" :class="sel == t ? 'border-4' : ''"
                        ...
                        <button @click.stop="handlerDelete(t)"
...
export default {
    ...
    methods: {
        add() {
            ...
            setInterval(async () => {
                const f = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${newTicker.name}&tsyms=USD&api_key=6fa93b3b50e8b1702eda309ff77ccf8eabaaccbb4483c54598eb415ca332e681`);

                const data = await f.json();

                this.tickers.find(t => newTicker.name == t.name).price = data.USD > 1 ? data.USD.toFixed(2) : data.USD.toPrecision(2);
